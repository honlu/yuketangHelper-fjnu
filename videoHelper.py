...
csrftoken = "your" #需改成自己的
sessionid = "your" #需改成自己的
...
headers = {
    ...
    'Cookie': 'csrftoken=' + csrftoken + '; sessionid=' + sessionid + '; university_id=3323; platform_id=3',
...
def one_video_watcher(video_id,video_name,cid,user_id,classroomid,skuid):
    ...
    url = "https://yjsy-fjnu.yuketang.cn/video-log/heartbeat/"
    get_url = "https://yjsy-fjnu.yuketang.cn/video-log/get_video_watch_progress/?cid="+str(cid)+"&user_id="+user_id+"&classroom_id="+classroomid+"&video_type=video&vtype=rate&video_id=" + str(video_id) + "&snapshot=1&term=latest&uv_id=3323"
...
def get_videos_ids(course_name,classroom_id,course_sign):
    get_homework_ids = "https://yjsy-fjnu.yuketang.cn/mooc-api/v1/lms/learn/course/chapter?cid="+str(classroom_id)+"&term=latest&uv_id=3323&sign="+course_sign
...
if __name__ == "__main__":
    ...
    user_id_url = "https://yjsy-fjnu.yuketang.cn/edu_admin/check_user_session/"
    id_response = requests.get(url=user_id_url, headers=headers)
    try:
        user_id = re.search(r'"user_id":(.+?)}', id_response.text).group(1).strip()
        print("测试:"+str(user_id))
    except:
        print("也许是网路问题，获取不了user_id,请试着重新运行")
        raise Exception("也许是网路问题，获取不了user_id,请试着重新运行!!! please re-run this program!")

    # 然后要获取教室id
    get_classroom_id = "https://yjsy-fjnu.yuketang.cn/mooc-api/v1/lms/user/user-courses/?status=1&page=1&no_page=1&term=latest&uv_id=3323"
    submit_url = "https://yjsy-fjnu.yuketang.cn/mooc-api/v1/lms/exercise/problem_apply/?term=latest&uv_id=3323"
